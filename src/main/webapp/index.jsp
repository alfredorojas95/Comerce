
            
            <div id="indexLeftColumn">
                <p>Texto izquierda</p>
                <ul>
                    <li><a href="categorias">Categoria</a></li>
                    <li><a href="nuevaCategoria">Nueva Categoria</a></li><br>
                    <li><a href="productos">Lista Productos</a></li>
                    <li><a href="nuevoProducto">Nuevo Producto</a></li><br>
                    <li><a href="usuarios">Usuarios</a></li>
                    <li><a href="nuevoUsuario">Nuevo Usuario</a></li><br>
                    <li><a href="ordenes">Listado Ordenenes</a></li>
                    <li><a href="nuevaOrden">Nueva Orden</a></li><br>
                    <li><a href="productoOrden">Producto Orden</a></li>
                    <li><a href="nuevoProductoOrden">Agregar producto Orden</a></li>
                    
                   
                </ul>
            </div>
            <div id="indexRightColumn">
                <div class="categoryBox">frutas</div>
                <div class="categoryBox">verduras</div>
                <div class="categoryBox">Lacteos</div>
                <div class="categoryBox">carne</div>
            </div>
            
          